codeunit 81900 "wanaStart Import FR"
{
    ...
    begin
        ...
        CsvBuffer.SetFilter("Field No.", '1|4|5|7|9..14');
    ...
    begin
        Evaluate(ReturnValue, CsvBuffer.Value);
    end;

    local procedure ToDate(pCell: Text) ReturnValue: Date
    begin
        Evaluate(ReturnValue, CsvBuffer.Value.Substring(7, 2) + CsvBuffer.Value.Substring(5, 2) + CsvBuffer.Value.Substring(1, 4));
    end;
    ...
    begin
        case pColumnNo of
            1: // JournalCode
                pRec.Validate("Source Code", ToSourceCode(pCell));
            2: // JournalLib
                ;
            3: // EcritureNum
                ;
            ...
            5: // CompteNum
                _Account."From Account No." := pCell;
            6: // CompteLib
                ;
            7: // CompteAuxNum
                _Account."From SubAccount No." := pCell;
            8: // CompteAuxLib
                ;
            9: // PieceRef
                begin
                    MapAccount(pRec);
                    pRec.Validate("Document No.", CopyStr(pCell, 1, maxstrlen(pRec."Document No.")));
                end;
            10: // PieceDate
                pRec.Validate("Document Date", ToDate(pCell));
            11: // EcritureLib
                //if pRec.Description = '' then
                pRec.Validate(Description, CopyStr(pCell, 1, maxstrlen(pRec.Description)));
            //else
            //    pRec.Description := CopyStr(pRec.Description + pCell, 1, MaxStrLen(pRec.Description));
            12: // Debit
                pRec.Validate("Debit Amount", ToDecimal(pCell));
            13: // Credit
                Case pCell of
                    'D':
                        ;
                    'C':
                        ...
                        pRec.Validate("Credit Amount", ToDecimal(pCell));
                end;
            14: // EcritureLet
                pRec.Validate("Applies-to ID", CopyStr(pCell, 1, MaxStrLen(pRec."Applies-to ID")));
            15: // DateLet
                ;
            16: // MontantDev
                ;
