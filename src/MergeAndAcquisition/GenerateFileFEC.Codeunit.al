// from codeunit 10826 "Generate File FEC"
...
codeunit 87136 "WanaStart Generate File M&A"
{
    Access = Internal;
    ...
    var
        ...
        DataHandlingFEC: Codeunit "WanaStart Export Data Handling";
        ...
        */
        PayablesAccounts: List of [Code[20]];
        ReceivablesAccounts: List of [Code[20]];
        //]
    ...
    */
    ...
    begin
        GLAccount.SetLoadFields("Account Type", Name, "Detailed Balance");
        SetFiltersGLAccount(GLAccount, AuditFileExportHeader."G/L Account View String");
        GLAccount.SetFilter("Date Filter", '..%1', ClosingDate(AuditFileExportHeader."Starting Date" - 1));
        GLAccount.SetAutoCalcFields("Balance at Date");

        if GLAccount.FindSet() then
            repeat
                TotalDetailedBalance := 0;
                if GLAccount."Detailed Balance" then
                    wanWriteDetailedOpeningBalance(GLAccount."No.", GLAccount.Name, AuditFileExportHeader."Starting Date", TotalDetailedBalance);

                if GLAccount."Balance at Date" - TotalDetailedBalance <> 0 then
                    WriteGLAccount(GLAccount."No.", GLAccount.Name, GLAccount."Balance at Date" - TotalDetailedBalance);
            until GLAccount.Next() = 0;
    end;
    //]
    ...
    begin
        ...
        //[
        GLEntry.AddLoadFields("External Document No.", "Global Dimension 1 Code", "Global Dimension 2 Code", "Dimension Set ID");
        GLEntry.SetAutoCalcFields("G/L Account Name");
        //]
    ...
    */
    ...
    begin
        ...
        case GLEntry."Source Type" of
            GLEntry."Source Type"::Customer:
                if ReceivablesAccounts.Contains(GLEntry."G/L Account No.") then begin
                    Customer.SetLoadFields(Name);
                    Customer.Get(GLEntry."Source No.");
                    PartyNo := Customer."No.";
                    PartyName := Customer.Name;
                    CustLedgerEntry.SetLoadFields(Description, "Posting Date", "Document Date", "Document No.", "Closed by Entry No.", "Closed at Date", "External Document No.");
                    if not CustLedgerEntry.Get(GLEntry."Entry No.") then
                        FindPrevCustLedgerEntry(CustLedgerEntry, GLEntry."Entry No.");
                    wanGetCustAppliesToID(CustLedgerEntry, DocNoApplied, DateApplied);
                    CurrencyCode := GLEntry."Source Currency Code";
                    FCYAmount := FormatAmount(GLEntry."Source Currency Amount");
                end;
            GLEntry."Source Type"::Vendor:
                if PayablesAccounts.Contains(GLEntry."G/L Account No.") then begin
                    Vendor.SetLoadFields(Name);
                    Vendor.Get(GLEntry."Source No.");
                    PartyNo := Vendor."No.";
                    PartyName := Vendor.Name;
                    VendorLedgerEntry.SetLoadFields(Description, "Posting Date", "Document Date", "Document No.", "Closed by Entry No.", "Closed at Date", "External Document No.");
                    if not VendorLedgerEntry.Get(GLEntry."Entry No.") then
                        FindPrevVendorLedgerEntry(VendorLedgerEntry, GLEntry."Entry No.");
                    wanGetVendorAppliesToID(VendorLedgerEntry, DocNoApplied, DateApplied);
                    CurrencyCode := GLEntry."Source Currency Code";
                    FCYAmount := FormatAmount(GLEntry."Source Currency Amount");
                end;
        end;
        ...
        if GLRegisterGlobal.SystemCreatedAt <> 0DT then
            DateCreated := DT2Date(GLRegisterGlobal.SystemCreatedAt)
        else
            DateCreated := GLRegisterGlobal."Creation Date";
    ...
    begin
        ...
        //[
        GLEntry.AddLoadFields("External Document No.", "Global Dimension 1 Code", "Global Dimension 2 Code");
        //]
    ...
    */
    ...
    begin
        if (EntryNo >= GLRegisterGlobal."From Entry No.") and (EntryNo <= GLRegisterGlobal."To Entry No.") then
            exit;
        GLRegisterGlobal.SetLoadFields("From Entry No.", "To Entry No.", "Creation Date");
        if EntryNo > GLRegisterGlobal."To Entry No." then
            GLRegisterGlobal.SetFilter("No.", '>%1', GLRegisterGlobal."No.");
        GLRegisterGlobal.SetFilter("From Entry No.", '<=%1', EntryNo);
        GLRegisterGlobal.SetFilter("To Entry No.", '>=%1', EntryNo);
        GLRegisterGlobal.FindFirst();
    end;
        ...
        case SourceType of
            GLSourceType::Customer:
                begin
                    ...
                    //[
                    CustLedgerEntry.AddLoadFields(Open, "Closed by Entry No.", "Closed at Date");
                    //]
            ...
            GLSourceType::Vendor:
                begin
                    ...
                    //[
                    VendorLedgerEntry.AddLoadFields(Open, "Closed by Entry No.", "Closed at Date");
                    //]
    ...
    */
    ...
    begin
        ...
        */
           'PieceDate|EcritureLib|Debit|Credit|EcritureLet|DateLet|ValidDate|Montantdevise|Idevise' +
           '|_ExternalDocumentNo|_ShortcutDimension1Code|_ShortcutDimension2Code');
        //]
    end;
    ...
    begin
        //[
        // GLEntry.CalcFields(GLEntry."G/L Account Name"); +SetAutoCalcFields
        //]
    ...
    //[
    local procedure SetReceivablesAccount();
    var
        CustomerPostingGroup: Record "Customer Posting Group";
    begin
        CustomerPostingGroup.SetLoadFields("Receivables Account");
        if CustomerPostingGroup.FindSet() then
            repeat
                ReceivablesAccounts.Add(CustomerPostingGroup."Receivables Account");
            until CustomerPostingGroup.Next() = 0;
    end;

    local procedure SetPayablesAccounts();
    var
        VendorPostingGroup: Record "Vendor Posting Group";
    begin
        VendorPostingGroup.SetLoadFields("Payables Account");
        if VendorPostingGroup.FindSet() then
            repeat
                PayablesAccounts.Add(VendorPostingGroup."Payables Account");
            until VendorPostingGroup.Next() = 0;
    end;

    local procedure wanWriteDetailedOpeningBalance(pGLAccountNo: Code[20]; pGLAccountName: Text; pStartingDate: Date; var TotalDetailedBalance: Decimal)
    var
        GLEntry: Record "G/L Entry";
    begin
        GLEntry.SetLoadFields("Posting Date", "Source Type", "Source No.");
        GLEntry.SetRange("G/L Account No.", pGLAccountNo);
        GLEntry.SetRange("Posting Date", 0D, pStartingDate - 1);
        GLEntry.SetFilter("Source Type", '%1|%2', "Gen. Journal Source Type"::Customer, "Gen. Journal Source Type"::Vendor);
        if GLEntry.FindSet() then
            repeat
                case GLEntry."Source Type" of
                    "Gen. Journal Source Type"::Customer:
                        wanOpeningCustLedgerEntry(GLEntry, pGLAccountName, pStartingDate, TotalDetailedBalance);
                    "Gen. Journal Source Type"::Vendor:
                        wanOpeningVendorLedgerEntry(GLEntry, pGLAccountName, pStartingDate, TotalDetailedBalance);
                end;
            until GLEntry.Next() = 0;
    end;

    local procedure wanOpeningCustLedgerEntry(pGLEntry: Record "G/L Entry"; pGLAccountName: Text; pStartingDate: Date; var TotalDetailedBalance: Decimal)
    var
        PartyLedgerEntry: Record "Cust. Ledger Entry";
        Party: Record Customer;
        CVLEB: Record "CV Ledger Entry Buffer";
        DocNoSet: Text;
        DateApplied: Date;
    begin
        PartyLedgerEntry.SetLoadFields("Customer No.", "Currency Code", "Remaining Amount", "Remaining Amt. (LCY)", Description, "Document Date", "Document No.", "External Document No.");
        PartyLedgerEntry.SetFilter("Posting Date", '<%1', pStartingDate);
        PartyLedgerEntry.SetFilter("Date Filter", '<%1', pStartingDate);
        PartyLedgerEntry.SetAutoCalcFields("Remaining Amount", "Remaining Amt. (LCY)");
        if PartyLedgerEntry.Get(pGLEntry."Entry No.") and (PartyLedgerEntry."Remaining Amt. (LCY)" <> 0) then begin
            Party.SetLoadFields(Name);
            Party.Get(pGLEntry."Source No.");
            CVLEB.CopyFromCustLedgEntry(PartyLedgerEntry);
            wanGetCustAppliesToID(PartyLedgerEntry, DocNoSet, DateApplied);
            wanWriteOpenEntry(pGLEntry."G/L Account No.", pGLAccountName, pGLEntry."Source No.", Party.Name, CVLEB, PartyLedgerEntry.SystemCreatedAt, DocNoSet, DateApplied);
            TotalDetailedBalance += PartyLedgerEntry."Remaining Amt. (LCY)";
        end;
    end;

    local procedure wanOpeningVendorLedgerEntry(pGLEntry: Record "G/L Entry"; pGLAccountName: Text; pStartingDate: Date; var TotalDetailedBalance: Decimal)
    var
        PartyLedgerEntry: Record "Vendor Ledger Entry";
        Party: Record Vendor;
        CVLEB: Record "CV Ledger Entry Buffer";
        DocNoSet: Text;
        DateApplied: Date;
    begin
        PartyLedgerEntry.SetLoadFields("Vendor No.", "Currency Code", "Remaining Amount", "Remaining Amt. (LCY)", Description, "Document Date", "Document No.", "External Document No.");
        PartyLedgerEntry.SetFilter("Posting Date", '<%1', pStartingDate);
        PartyLedgerEntry.SetFilter("Date Filter", '<%1', pStartingDate);
        PartyLedgerEntry.SetAutoCalcFields("Remaining Amount", "Remaining Amt. (LCY)");
        if PartyLedgerEntry.Get(pGLEntry."Entry No.") and (PartyLedgerEntry."Remaining Amt. (LCY)" <> 0) then begin
            Party.SetLoadFields(Name);
            Party.Get(pGLEntry."Source No.");
            CVLEB.CopyFromVendLedgEntry(PartyLedgerEntry);
            wanGetVendorAppliesToID(PartyLedgerEntry, DocNoSet, DateApplied);
            wanWriteOpenEntry(pGLEntry."G/L Account No.", pGLAccountName, pGLEntry."Source No.", Party.Name, CVLEB, PartyLedgerEntry.SystemCreatedAt, DocNoSet, DateApplied);
            TotalDetailedBalance += PartyLedgerEntry."Remaining Amt. (LCY)";
        end;
    end;

    local procedure wanWriteOpenEntry(GLAccountNo: Code[20]; GLAccountName: Text[100]; SourceNo: Code[20]; PartyName: Text[100]; CVLEB: Record "CV Ledger Entry Buffer"; pSystemCreatedAt: DateTime; pDocNoSet: Text; pDateApplied: Date)
    var
        DebitAmt: Decimal;
        CreditAmt: Decimal;
    begin
        if CVLEB."Remaining Amt. (LCY)" > 0 then
            DebitAmt := CVLEB."Remaining Amt. (LCY)"
        else
            CreditAmt := -CVLEB."Remaining Amt. (LCY)";
        if CVLEB."Currency Code" = '' then
            CVLEB."Remaining Amount" := 0;
        ...
        AppendLine('00000|' +
          ...
          FormatAmount(CreditAmt) + '|' +
          pDocNoSet + '|' +
          DataHandlingFEC.GetFormattedDate(pDateApplied) + '|' +
          DataHandlingFEC.GetFormattedDate(pSystemCreatedAt.Date) + '|' +
          FormatAmount(CVLEB."Remaining Amount") + '|' +
          CVLEB."Currency Code" + '|' +
          CVLEB."External Document No." + '|' +
          CVLEB."Global Dimension 1 Code" + '|' +
          CVLEB."Global Dimension 2 Code");
    end;

    local procedure wanGetCustAppliesToID(pLedgerEntry: Record "Cust. Ledger Entry"; var pDocNoSet: Text; var pDateApplied: Date): Integer
    begin
        if pLedgerEntry.Open then
            exit;
        if pLedgerEntry."Closed by Entry No." = 0 then begin
            pDocNoSet := Format(pLedgerEntry."Entry No.");
            pDateApplied := pLedgerEntry."Posting Date";
        end else if pLedgerEntry."Closed at Date" < EndingDate then begin
            pDocNoSet := Format(pLedgerEntry."Closed by Entry No.");
            pDateApplied := pLedgerEntry."Closed at Date";
        end;
    end;

    local procedure wanGetVendorAppliesToID(pLedgerEntry: Record "Vendor Ledger Entry"; var pDocNoSet: Text; var pDateApplied: Date): Integer
    begin
        if pLedgerEntry.Open then
            exit;
        if pLedgerEntry."Closed by Entry No." = 0 then begin
    ...
    local procedure FindPrevVendorLedgerEntry(var pLedgerEntry: Record "Vendor Ledger Entry"; pEntryNo: Integer)
    begin
        pLedgerEntry.SetRange("Entry No.", 0, pEntryNo);
        pLedgerEntry.FindLast();
        pLedgerEntry.SetRange("Entry No.", 0, pEntryNo);
    end;

    local procedure FindPrevCustLedgerEntry(var pLedgerEntry: Record "Cust. Ledger Entry"; pEntryNo: Integer)
    begin
        pLedgerEntry.SetRange("Entry No.", 0, pEntryNo);
        pLedgerEntry.FindLast();
        pLedgerEntry.SetRange("Entry No.", 0, pEntryNo);
