...
codeunit 87100 "wanaStart Import FR"
{
    ...
    begin
        if MapAccount.Skip or MapSourceCode.Skip or
            ...
            (MapAccount."Gen. Posting Type" = MapSourceCode."Gen. Posting Type") and
            SkipVAT then
            exit;
        if MapSourceCode."Gen. Posting Type" <> MapSourceCode."Gen. Posting Type"::" " then begin
            ...
            if pRec."Account Type" in [pRec."Account Type"::Vendor, pRec."Account Type"::Customer] then begin
                SkipVAT := pRec."Applies-to ID" = '';
                if SkipVAT then begin
    ...
    begin
        case CsvBuffer."Field No." of
            ...
            9: // PieceRef
                begin
                    ToAccount(pRec);
                    if MapSourceCode."Start" then //TODO PieceRef to Document No., et Quid si pas de PieceRef (case 9 non déclenché)
